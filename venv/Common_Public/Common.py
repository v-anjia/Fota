...
from Common_Public import LogCatFile as lcf
from Common_Public import ScreenCap as scp
...
class Serial(object):
    ...
    def open_adb_through_serial(self,count):
        ...
        time.sleep(5)
        ser.write(("ps -ef|grep settings |grep -v grep| %s  awk '{print $2}'|xargs kill -9 \r\n" %(busybox)).encode('utf-8'))
        time.sleep(5)
        ...
        time.sleep(5)
        ser.write("input swipe 300 1300 500 1300 1000\r\n".encode('utf-8'))
        time.sleep(5)
...
class Install_Package(object):
    ...
    def update_fota_package(self):
        ...
        '''
        filelist = []
        file_name = os.path.join(os.path.dirname(__file__),'date_for_fota')
...
def Get_libHU_Message(sn):
    try:
        ...
        os.system('adb -s %s shell "mkdir -p /update/tbox/"' %(sn))
        ...
        os.system('adb -s %s shell "adb pull %s /update/tbox/"' % (sn, tbox_log_path))
        os.system("adb -s %s pull /update/tbox/.  %s/%s_%s" \
                  %(sn,libHU_path,time.strftime('%Y-%m-%d__%H-%M-%S', time.localtime(time.time())), "update_log"))
    except Exception as e:
        logger.log_error("%s" %(e),\
                         sys._getframe().f_code.co_filename, sys._getframe().f_code.co_name, sys._getframe().f_lineno)
