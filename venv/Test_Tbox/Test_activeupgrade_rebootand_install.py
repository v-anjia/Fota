...
def test():
    logger.log_info("start tbox active upgrade reboot and install through ui", \
    ...
    count_pass = 0
    count_fail = 0
    Result = tbox_common.check_reboot_and_install(sn, "mcu", "mpu")
    if Result == 0:
...
# 
#     logger.log_info("start active upgrade reboot and install through ui", \
#                     sys._getframe().f_code.co_filename, sys._getframe().f_code.co_name, sys._getframe().f_lineno)
#     count_pass = 0
#     count_fail = 0
#     sn = tbox_common.check_sn_status()
